...
namespace Graph{
    ...
    struct HeadNode{
        ...
        explicit HeadNode(const VertInfo& data_): data(data_), adj(std::list<EdgeNode>()) {}
    };
    ...
    const int& VertexCount() const{
        return n_verts;    // 获取顶点数
    }
    const int& EdgesCount() const{
        return m_edges;    // 获取边数
    }
    const std::vector<HeadNode>& List() const{
        return ver_list;    // 获取邻接表
    }
    const std::map<std::string, Vertex>& Map() const{
        return vertex_map;    //获取映射表
    }
